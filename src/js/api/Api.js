...
const urlConverter = (values= {}, page = 1) => {
    ...
    let startDate = values.startDate ? '&begin_date=' + values.startDate.format().slice(0, 10).split('-').join('') : '';
    let endDate = values.endDate ? '&end_date=' + values.endDate.format().slice(0, 10).split('-').join('') : '';
...
const NYTApi = (values, page) => {
    console.log(values.startDate.format());
